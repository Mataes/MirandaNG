...
int LoadMainOptions()
{
	...
	if ( db_get_b(NULL, MODULENAME, SETTING_CHECKCONNECTION, 1)) {
		...
		if ( db_get_b(NULL, MODULENAME, SETTING_CHECKAPMRESUME, 1)&&(CallService(MS_SYSTEM_GETVERSION,0,0) >= 0x00040000)) {
			if (!IsWindow(hMessageWindow)) {
...
static INT_PTR ShowPopup(TCHAR *msg, HICON hIcon)
{
	...
	else if ( !db_get_b(NULL, MODULENAME, SETTING_POPUP_USEDEFCOLORS, 1))
	{
