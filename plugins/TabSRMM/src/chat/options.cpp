...
static ColorOptionsList _clrs[] = {
	...
	{ 0, LPGENT("Message Sessions")_T("/")LPGENT("Info Panel"), LPGENT("Panel background low"), "ipfieldsbg", RGB(255, 202, 98) },
	{ 1, LPGENT("Message Sessions")_T("/")LPGENT("Info Panel"), LPGENT("Panel background high"), "ipfieldsbgHigh", 0xff000000 | COLOR_3DFACE },
...
static branch_t branch1[] = {
	...
	{ LPGENT("Show topic as status message on the contact list"), "TopicOnClist", 0, 0, NULL },
...
INT_PTR CALLBACK DlgProcOptions2(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		TranslateDialogDefault(hwndDlg);
		{
			...
			SendDlgItemMessage(hwndDlg, IDC_CHAT_SPIN4, UDM_SETPOS, 0, MAKELONG(db_get_w(NULL, CHAT_MODULE, "LoggingLimit", 0), 0));
...
INT_PTR CALLBACK DlgProcOptions3(HWND hwndDlg, UINT uMsg, WPARAM, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		TranslateDialogDefault(hwndDlg);
		{
			...
			DWORD dwTrayFlags = M.GetDword(CHAT_MODULE, "TrayIconFlags", GC_EVENT_ALL);
			DWORD dwPopupFlags = M.GetDword(CHAT_MODULE, "PopupFlags", GC_EVENT_ALL);
