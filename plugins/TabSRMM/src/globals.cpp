...
static TContainerSettings _cnt_default = { false, CNT_FLAGS_DEFAULT, CNT_FLAGSEX_DEFAULT, 255, CInfoPanel::DEGRADE_THRESHOLD, 60, _T("%n - %s %x"), 2, 1 };
...
void CGlobals::reloadSettings(bool fReloadSkins)
{
	...
	m_bStatusOnTabs = M.GetBool("tabstatus", false);
	...
	m_bHideOnClose = M.GetBool("hideonclose", true);
	m_bAllowTab = M.GetBool("tabmode", false);

	m_bFlashOnClist = M.GetBool("flashcl", true);
	m_bAlwaysFullToolbarWidth = M.GetBool("alwaysfulltoolbar", true);
	m_LimitStaticAvatarHeight = M.GetDword("avatarheight", 128);
	m_SendFormat = M.GetByte("sendformat", 1);
...
{
	m_bSoundOnTyping = M.GetBool("adv_soundontyping", true);
