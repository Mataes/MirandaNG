...
void CMimAPI::InitPaths()
{
	...
	mir_sntprintf(m_szProfilePath, SIZEOF(m_szProfilePath), _T("%sAvatars\\TabSRMM"), szUserdataDir);
	if (ServiceExists(MS_FOLDERS_REGISTER_PATH)) {
	...
	mir_sntprintf(m_szSavedAvatarsPath, SIZEOF(m_szSavedAvatarsPath), _T("%s"), m_szProfilePath);
}
...
int CMimAPI::MessageEventAdded(WPARAM hContact, LPARAM hDbEvent)
{
	...
	bool bAutoCreate = M.GetBool("autotabs", false);
	bool bAutoContainer = M.GetBool("autocontainer", false);
