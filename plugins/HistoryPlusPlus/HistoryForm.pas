...
begin
  ...
  SearchMode := TSearchMode(GetDBByte(hppDBName, 'SearchMode', 1));
end;
...
begin
  ...
  hg.AddItem;
  if HistoryLength = 1 then
    if GetDBBool(hppDBName, 'ShowSessions', True) and not(hpSessions in Panel) then
...
begin
  ...
  begin
    if GetDBBool(hppDBName, 'ShowSessions', True) then
      include(tPanel, hpSessions);
    if GetDBBool(hppDBName, 'ShowBookmarks', True) then
